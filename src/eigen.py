...
# Check arguments passed to the parser and throw descriptive errors
def checkArgs(args):

	itemCatPotential = {'well', 'harmonic'}

	if args['N'] < 0:
		raise argparse.ArgumentTypeError("N must be greater than 0.")
	if args['potential'] not in itemCatPotential:
		raise argparse.ArgumentTypeError("Choose either 'well' or 'harmonic'.")
	if args['n_eigs'] > (args['N'] ** 2):
		raise argparse.ArgumentTypeError("The value for n_eigs must be less than or equal to N squared.")
...
if __name__ == '__main__':
	...
	parser.add_argument('potential', type=str, help="Type of potential")
	parser.add_argument('n_eigs', type=int, help="Number of eigenvalues (Integer)")

	# Get the input arguments
	args = vars(parser.parse_args())	
	
	# Validate the arguments passed and throw errors otherwise
	checkArgs(args)
